...
### 3-7 المعمارية الأمنية المحلية وقفل PIN (Offline Local Security Architecture)

التزاماً بمبدأ العمل دون اتصال أولاً (Offline-First) وحماية خصوصية المستخدم المالية دون الحاجة لحساب سحابي أو شبكة، صُممت معمارية الأمان وفق مبادئ العمارة النظيفة (Clean Architecture):

```
┌──────────────────────────────────────────────────────────┐
│                   PRESENTATION LAYER                     │
│  PinLockScreen (Keypad, Haptic, 5-Attempt 30s Lockout)   │
│  SettingsScreen (Security Toggle & Navigation)          │
└────────────────────────────┬─────────────────────────────┘
                             │ calls
┌────────────────────────────▼─────────────────────────────┐
│                      DOMAIN LAYER                        │
│  PinCode (Pure Value Object: 4 digits, SHA-256 Hashing)  │
│  SecurityRepository (Pure Abstract Contract)             │
│  VerifyPinUseCase · SetPinUseCase · DisablePinUseCase    │
└────────────────────────────▲─────────────────────────────┘
                             │ implements
┌────────────────────────────┴─────────────────────────────┐
│                       DATA LAYER                         │
│  SecurityRepositoryImpl (Translates SQLite Exceptions)   │
│  SecurityLocalDataSource (Manages settings table)        │
│  Settings Table (pin_hash: Text, is_pin_enabled: Bool)  │
└──────────────────────────────────────────────────────────┘
```

1. **كائن القيمة النقي `PinCode` (Domain Value Object):**
   - مسؤول عن التحقق الصارم من أن الرمز يتكون بدقة من 4 أرقام عددية [0-9].
   - تشفير الرمز حتمياً باستخدام دالة التجزئة الآمنة **SHA-256** بدون ملح خارجي للحفاظ على استقلالية التحقق المحلي.
   - خلو كامل من أي تبعية لـ Flutter أو مكتبات التخزين.
2. **عقود حالات الاستخدام (Domain Use Cases):**
   - `VerifyPinUseCase`: يقارن بين المرشح المدخل والتجزئة المخزنة.
   - `SetPinUseCase`: يحفظ التجزئة الجديدة ويفعل وضع القفل.
   - `DisablePinUseCase`: يمسح التجزئة ويعطل القفل بعد التحقق.
   - `IsPinEnabledUseCase`: يستعلم عن حالة التفعيل الحالية.
3. **طبقة البيانات والمثابرة المحلية (Data Persistence):**
   - تخزين التجزئة `pin_hash` وحالة القفل `is_pin_enabled` داخل جدول `settings` الأحادي في Drift/SQLite.
   - معالجة أي استثناءات لقاعدة البيانات وتحويلها إلى `DatabaseFailure` دون تسريب استثناءات تقنية.
4. **شاشة القفل وحماية التخمين (Presentation Security Gate):**
   - تُعرض `PinLockScreen` كبوابة دخول عند تشغيل التطبيق في `lib/main.dart` إذا كان القفل مفعلاً.
   - تدعم 3 أوضاع: فتح التطبيق (`unlock`)، وإنشاء الرمز مع التأكيد المزدوج (`create`)، وتعطيل القفل بعد التحقق (`disable`).
   - **آلية الحظر الوقائي (Lockout Defense):** عند حدوث 5 محاولات إدخال خاطئة متتالية، يُعطل الإدخال فورياً لمدة 30 ثانية مع عداد تنازلي واهتزاز تحذيري.

---

### 3-8 بنية التحليلات والرسوم البيانية التفاعلية (fl_chart Integration)

تم دمج مكتبة `fl_chart: ^1.2.0` داخل طبقة العرض (`presentation/widgets`) مع الالتزام التام بفصل الاهتمامات:
- **`CategoryPieChart`:** رسم بياني دائري تفاعلي يمثل توزيع المصاريف الشهرية حسب الفئات، يدعم تكبير الشريحة المختارة باللمس مع عرض مركز الدائرة لإجمالي العملة ودليل توضيحي مصاحب.
- **`MonthlyComparisonChart`:** رسم بياني شريطي يقارن مبالغ الشهر الكلية بما تم تسديده فعلياً والمتبقي تسديده، مع شارة نسبة مئوية ديناميكية.
- **العزل المعماري:** تعمل الرسوم البيانية حصرياً كعناصر واجهة (Widgets) تعتمد على بيانات الدومين الجاهزة الممررة من `SummaryController`، دون أي نفاذ مباشر إلى قاعدة البيانات.
