...
| Arabic/RTL text, localization keys, number and date formats    | `flutter-setup-localization`, `ui-ux-pro-max`, `recurring-commitments-expert` | Strict Arabic pluralization rules (ArabicPluralFormatter) |
| PIN security, price history audit trail, or fl_chart analytics | `recurring-commitments-expert`, `clean-architecture` | 100% offline security and reactive charts             |
...
Skills tell the agent _how_ to write the code; the project documents tell it _what is allowed_. For every task the agent also opens the matching document: `docs/ARCHITECTURE.md` for structure and data-model questions, `docs/TEST_PLAN.md` for test design, `docs/RISK_REGISTER.md` for money, time, storage, permissions, backup and destructive actions, and `docs/KANBAN_AND_GIT_WORKFLOW.md` before opening any branch or pull request.
